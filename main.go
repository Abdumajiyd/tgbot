...
func (b *Bot) downloadAndSendVideo(url string) {
	log.Println("Videoni yuklab olish jarayoni boshlandi:", url)
	...
	log.Println("Video muvaffaqiyatli yuklab olindi:", videoPath)
	b.sendMessage("Video muvaffaqiyatli yuklab olindi, sizga jo'natilyapti")
	b.sendVideo(videoPath)

	// Faylni yuborgandan keyin o'chirish
	defer func() {
		log.Println("Video fayl o'chirilmoqda:", videoPath)
		err := os.Remove(videoPath)
		if err != nil {
			log.Println("Video faylini o'chirishda xatolik yuz berdi:", err)
		} else {
			log.Println("Video fayli muvaffaqiyatli o'chirildi:", videoPath)
		}
	}()
}
...
func (b *Bot) sendMessage(text string) {
	...
	if err != nil {
		...
		log.Println("Xabar muvaffaqiyatli yuborildi:", text)
	}
}
